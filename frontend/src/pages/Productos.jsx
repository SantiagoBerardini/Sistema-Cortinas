...
export default function Productos() {
    ...
    const abrirRegistrarCorte = (producto) => {
    ...
    return (
        <>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="form-grid">
                                ...
                                <div className="form-group">
                                    ...
                                    <select value={form.tipo_medida} onChange={e => setForm({ ...form, tipo_medida: e.target.value })}>
                                ...
                                {form.tipo_medida === 'metros_cuadrados' && (
                                    <>
                                        <div className="form-group">
                                            ...
                                            <input type="number" step="0.1" value={form.ancho_rollo || ''} onChange={e => setForm({ ...form, ancho_rollo: parseFloat(e.target.value) || 0 })} placeholder="ej: 2.5" />
                                        </div>
                                        <div className="form-group">
                                            <label>Largo del rollo (m)</label>
                                            <input type="number" step="0.1" value={form.largo_rollo || 30} onChange={e => setForm({ ...form, largo_rollo: parseFloat(e.target.value) || 30 })} placeholder="ej: 30" />
                                        </div>
                                        <div className="form-group">
                                            <label>Factor desperdicio</label>
                                            <input type="number" step="0.01" min="1.0" max="1.5" value={form.factor_desperdicio || 1.0} onChange={e => setForm({ ...form, factor_desperdicio: parseFloat(e.target.value) || 1.0 })} placeholder="ej: 1.1" />
                                        ...
                                        <div className="form-group">
                                            ...
                                            <select value={form.es_producto_base ? 'si' : 'no'} onChange={e => setForm({ ...form, es_producto_base: e.target.value === 'si' })}>
                                ...
                                <div className="form-group">
                                    ...
                                    <input type="number" value={form.stock_actual} onChange={e => setForm({ ...form, stock_actual: parseFloat(e.target.value) || 0 })} />
                                </div>
                                <div className="form-group">
                                ...
                                <div className="form-group">
                                    ...
                                    <input type="number" value={form.costo_unitario} onChange={e => setForm({ ...form, costo_unitario: parseFloat(e.target.value) || 0 })} />
                                </div>
                            ...
                            <div style={{ display: 'flex', gap: 8, marginTop: 16 }}>
                                ...
                                <button className="btn" type="button" style={{ background: '#f0ede8', color: '#6b6560' }} onClick={cerrarModal}>
                                    Cancelar
                                </button>
                            </div>
                        </form>
            ...
            {showModalCorte && productoSeleccionado && (
                <RegistrarCorte producto={productoSeleccionado} onClose={cerrarModalCorte} />
            )}
            ...
            <div className="card" style={{ padding: '16px 24px' }}>
                <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'flex-end' }}>
                    <div className="form-group" style={{ flex: 2, minWidth: 180 }}>
                        <label>Buscar</label>
                        <input placeholder="Nombre o SKU..." value={busqueda} onChange={e => setBusqueda(e.target.value)} />
                    </div>
                    ...
                    {(busqueda || filtroCat || filtroStock) && (
                        <button className="btn" style={{ background: '#f0ede8', color: '#6b6560', marginBottom: 1 }} onClick={() => { setBusqueda(''); setFiltroCat(''); setFiltroStock('') }}>
                            Limpiar
                        </button>
                    )}
                </div>
                ...
                <table className="tabla-productos">
                    ...
                    <tbody>
                        {productosFiltrados.map(p => (
                            <tr key={p.id} style={{ opacity: p.es_producto_base ? 1 : 0.6, fontStyle: !p.es_producto_base ? 'italic' : 'normal' }}>
                                <td>
                                    <div>{p.nombre}</div>
                                    {!p.es_producto_base && (<small style={{ color: '#999' }}>(Subproducto)</small>)}
                                </td>
                                ...
                                <td>{p.factor_desperdicio ? `${((p.factor_desperdicio - 1) * 100).toFixed(0)}%` : '-'}</td>
                                <td>
                                    {p.stock_actual <= p.stock_minimo ?
                                        <span style={{ backgroundColor: '#fee', color: '#c33', padding: '2px 6px', borderRadius: '3px', fontSize: '0.85em', fontWeight: 'bold' }}>BAJO</span>
                                        : <span style={{ backgroundColor: '#efe', color: '#3c3', padding: '2px 6px', borderRadius: '3px', fontSize: '0.85em' }}>OK</span>
                                    }
                                </td>
                                <td style={{ display: 'flex', gap: 4 }}>
                                    <button className="btn btn-sm btn-secondary" onClick={() => abrirEditar(p)} title="Editar">✏️</button>
                                    {p.es_producto_base && (
                                        <button className="btn btn-sm" onClick={() => abrirRegistrarCorte(p)} title="Registrar corte" style={{ backgroundColor: '#e8650a', color: 'white', border: 'none', padding: '4px 8px', borderRadius: '3px', cursor: 'pointer' }}>
                                            🔪
                                        </button>
                                    )}
